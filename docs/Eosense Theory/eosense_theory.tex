...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{cXcc}
		...
		$C_a$ & Atmospheric CO$_2$ concentration & mol m$^{-3}$ & Measured \\
		
		$C_s$ & CO$_2$ concentration in Sample Node chamber & mol m$^{-3}$ & Measured \\
		
		$C_r$ & CO$_2$ concentration in Reference Node chamber & mol m$^{-3}$ & Measured \\
		
		$C_c$ & CO$_2$ concentration in collar & mol m$^{-3}$ & Computed \\
...
\begin{figure}[H]
	\includegraphics[width=1.0\linewidth]{Drawings/flux-chamber-diagram}
...
Combine the two time derivatives (Eq. \ref{eq:5} and \ref{eq:6}). This removes the dependence on $C_a$ and any bias from atmospheric variability, as the Reference Node acts as a real-time correction for atmospheric exchange.
...
\begin{equation} \label{eq:10}
	\boxed{f_c = \frac{V_m}{S_c} \left( \frac{\partial C_s}{\partial t} - \frac{\partial C_r}{\partial t} \right) + \frac{S_a}{S_c} k_a (C_s - C_r)}
\end{equation}

This equation depends only on:
\begin{itemize}
	\item  geometry ($S_a$, $S_c$, $V_m$),
...
Substituting in Eqs. \ref{eq:10} and \ref{eq:11}, the equation governing the CO$_2$ concentration in the collar (Eq. \ref{eq:9}) can be rewritten as
...
If we define the water-to-collar and  collar-to-chamber transfer rate constants as $\kappa_w = S_w k_w / V_c$ and $\kappa_c = S_c k_c / V_c$ (units: s$^{-1}$), respectively, Eq. \ref{eq:12}  becomes:

\begin{equation} \label{eq:15}
	\frac{\partial C_c}{\partial t} + ( \kappa_w + \kappa_c) C_c = \kappa_w C_w + \kappa_c C_s
\end{equation}

The general solution of the first-order, non-homogeneous linear differential equation (Eq. \ref{eq:13}) is given by the sum of the homogeneous and particular solutions:
...
Eqs. \ref{eq:16} and \ref{eq:20} depend on:
\begin{itemize}
	...
	\item the bottom-membrane gas transfer velocity ($k_c$), obtained via calibration (or using the averaged Eosense value; see Table \ref{tab:eosense-values}), and
...
\begin{equation}
	\left\{
	\begin{aligned}
		(\ref{eq:10}): f_c &= \frac{S_a}{S_c}\,k_a\,(C_s - C_a) \\
		(\ref{eq:13}): f_c &= k_c\,(C_c - C_s)                 \\
	\end{aligned}
	\right.
	\;\Longrightarrow\;
	C_c = C_s + \frac{S_a k_a}{S_c k_c}\,(C_s - C_r) \quad 
	\label{eq:20}
\end{equation}
...
First, substitute Eq. \ref{eq:20} into Eq. \ref{eq:21}. Given that $S_a = S_c = S_w$, this results in:
\begin{equation}\label{eq:22}
	k_w = \frac{k_a}{\dfrac{C_w - C_s}{C_s - C_r} - \dfrac{k_a}{k_c}}
\end{equation}
...
\begin{equation}\label{eq:24}
	\boxed{f_w^\dagger = \frac{k_a(C_w - C_r)}{\dfrac{C_w - C_s}{C_s - C_r} - \dfrac{k_a}{k_c}}}
\end{equation}
...
This can be illustrated as follows. From the Eqs. \ref{eq:3} and \ref{eq:5}, the Sample and Reference mass balances are, respectively:
\begin{equation*}
	V_m \frac{\partial C_s}{\partial t} = S_c f_c - S_a f_a
\end{equation*}
\begin{equation*}
...
The respective Sample and Reference mass balances become:
\begin{equation*}
	S_c k_c(C_s - C_c) = S_a k_a (C_s - C_a)
\end{equation*}
\begin{gather*}
	S_a k_a(C_r - C_a) = 0 \\
	\implies C_r = C_a
