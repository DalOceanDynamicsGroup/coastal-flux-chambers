...
## Connection to drifting logger
- Connection is via wifi, SSID `shopprinter`
- ssh connection `ssh flux@drifterpi.local`
- need to activate python virtual environment
  - `python ~/pythoncode/bin/activate`
- To run the data logging "headless" code is run via screen
  - `screen python loggingscript.py`
...
## To run python on reboot (this is out of date now)
