function poDat = processPO_current(selPath)

% Reads raw Pro-Oceanus Solu-Blu data using the current version of the DAQ
% code and outputs a timetable containing phase-averaged values. 
...
for i = 1:length(files)
    ...
    for j = 1:numel(lines)
        ...
        % New format has DAQ timestamp as first column
        if numel(parts) > 15 && contains(parts{1},'-')
            lines(j) = strtrim(strjoin(parts(2:end),','));
        end
    end

    % Remove blank/incomplete PO records
